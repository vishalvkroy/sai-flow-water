...
const addSampleProductsToProduction = async () => {
  try {
    if (!MONGODB_URI) {
    ...
    if (!seller) {
      console.error('❌ No seller/admin user found in production database');
      console.log('💡 You need to create a seller account first on production');
      process.exit(1);
    }
    ...
    console.log(`📦 Found ${existingCount} existing products in production`);
    ...
    // Insert sample products (or update if they exist)
    ...
    console.log(`✅ Successfully processed ${insertedProducts.length} products in production`);
    ...
    console.error('❌ Error adding sample products to production:', error);
  } finally {
...
if (require.main === module) {
  addSampleProductsToProduction();
}

module.exports = { addSampleProductsToProduction, sampleProducts };
